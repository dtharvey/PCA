...
if(show_plot == TRUE){
plot(xrot, yrot, pch = 19, 
     col = "black", cex = 0.75, asp = 1)
}
...
plot_rot_axes = function(file, show_rotated = TRUE,
  ...
  # set colors
  axis2_col = okabe_ito[3]
  axis1_col = okabe_ito[8]
  loadings_col = okabe_ito[4]
  origdata_col = okabe_ito[1]
  
  # determine maximum value for axis limits
  axis_max = max(c(file$x_range, file$y_range))
  
  # plot original data
  plot(x = file$x_original, y = file$y_original, 
       type = "p", asp = 1,
       xlim = c(-axis_max, axis_max), 
       ylim = c(-axis_max, axis_max),
       xlab = "variable 2", ylab = "variable 1", 
       pch = 19, col = origdata_col, 
       cex = 1.5)
  
  # add original axes as dashed lines
  abline(v = 0, lwd = 2, lty = 2, col = axis1_col)
  abline(h = 0, lwd = 2, lty = 2, col = axis2_col)

  # add rotated axes
  if(show_rotated == TRUE){
  if(file$angle == 0 | file$angle == 180){
    abline(h = 0, lwd = 3, lty = 1, col = axis2_col)
    abline(v = 0, lwd = 3, lty = 1, col = axis1_col)
  } else if(file$angle == 90){
    abline(h = 0, lwd = 3, lty = 1, col = axis1_col)
    abline(v = 0, lwd = 3, lty = 1, col = axis2_col)
  } else {
  ...
  points(x = file$axis1_x, y = file$axis1_y, 
         pch = 19, col = axis1_col, cex = 1)
  points(x = file$axis2_x, y = file$axis2_y, 
         pch = 19, col = axis2_col, cex = 1)
  }
  ...
  legend(x = "topleft", legend = c("variable 1", "variable 2"), 
         col = c(axis1_col, axis2_col), 
         lwd = c(3,3), lty = c(1,1), bty = "n") 
  }
  ...
  if(show_full_axes_legend == TRUE){
    ...
    legend(x = "topleft", legend = c("variances",axis1_text,axis2_text), 
           bg = "white", box.col = "white", text.col = c("black",axis1_col, axis2_col))
  }
  ...
  draw.arc(x = 0, y = 0, radius = axis_max/1.25, 
           deg1 = 90, deg2 = 90 - file$angle, 
           lwd = 2, col = axis1_col, lty = 1)
  text(x = -1, y = 8,
       labels = expression(Theta), cex = 2, col = axis1_col)
  draw.arc(x = 0, y = 0, radius = axis_max/1.25, 
           deg1 = 0, deg2 = 90 - file$angle, 
           lwd = 2, col = axis1_col, lty = 1)
  text(x = 8, y = -1,
       labels = expression(Phi), cex = 2, col = axis1_col)
  leg.txt <- c("loadings",
               paste0(expression(Theta), ": cos(",file$angle,") = ",
                      round(cos(file$angle * pi/180),digits = 3)),
               paste0(expression(Phi), ": cos(",file$angle-90,") = ",
                      round(cos((file$angle-90) * pi/180),digits = 3)))
  # TeX("$\\Theta : \\, cos(-60)$ = 0.50"),
  # TeX("$\\Phi : \\, cos(30)$ = 0.866"))
  legend(x = "bottomright", legend = leg.txt, cex = 1.25, bty = "n", text.col = c("black",axis1_col, axis1_col))
  }
...
raw_data = generate_data(angle = 30) 
